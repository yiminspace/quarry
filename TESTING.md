...
| R10 | visual | header adopts `VoyageToolbar` (voyage 0.8.0): DOM order is fixed 语言→明暗→调色板; lang button width is locked (`--vg-lang-w`) so 中/EN render the same box and toggling language never shifts the mode/palette buttons sideways; the three buttons agree on height/width/border-radius/box-sizing; the header's `.vg-toolbar` doesn't inherit the query-toolbar's padding/background/border-bottom chrome (shared class name with `ResultWorkbench`'s query toolbar) | test_gui_visual:test_header_toolbar_dom_order_is_lang_mode_palette, test_lang_switch_fixed_width_no_reflow, test_header_toolbar_buttons_share_full_box_spec, test_header_toolbar_has_no_query_toolbar_chrome | ✅ |
...
| 101 | header | compact tunnel keep-alive control: live status badge (`down`/`up`/`reconnecting`) + start/stop button backed by `/api/keepalive*`, matching `qy status` semantics | F:test_header_keepalive_badge_and_toggle; `/api/keepalive`, `/api/keepalive/up`, `/api/keepalive/down` unit-covered in test_gui_api.py | ✅ |
