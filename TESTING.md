...
| 106 | app shell | address bar + `document.title` track the active `db`/`env`/`table`; opening a focus URL (`?db=&env=&table=`, no `sql`) restores that selection without auto-running | F:test_selection_updates_url_and_title, F:test_focus_url_restores_selection_without_autorun, queryLink.test.ts | ✅ |
